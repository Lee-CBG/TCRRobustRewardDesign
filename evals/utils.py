...
def enc_list_bl_max_len(aa_seqs, blosum, max_seq_len):
    ...
    for seq in aa_seqs:
        ...
        for aa in seq:
            ...
            if aa in blosum:
                ...
                # Assign a zero vector for unknown amino acids
                e_seq[count] = np.zeros(len(blosum["A"]))
                count+=1
    ...
    # Prepare padded/truncated sequences
    n_seqs = len(aa_seqs)
    ...
    for i, seq in enumerate(sequences):
        ...
        if seq_len > max_seq_len:
            # Truncate the sequence from the middle
            half_len = max_seq_len // 2
            remaining = max_seq_len - half_len
            enc_aa_seq[i] = np.vstack((seq[:half_len], seq[-remaining:]))
        else:
            # Pad the sequence in the middle
            pad_len = max_seq_len - seq_len
            half_pad = pad_len // 2
            remaining_pad = pad_len - half_pad
            enc_aa_seq[i] = np.vstack((
                np.zeros((half_pad, n_features)),
                seq,
                np.zeros((remaining_pad, n_features))
            ))
