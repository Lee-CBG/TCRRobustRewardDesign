...
def nettcr_one_chain():
    cdr_in = Input(shape=(22,20))
    pep_in = Input(shape=(22,20))
...
from tensorflow.keras.layers import Input, Bidirectional, LSTM, Dense, Dropout, Concatenate, LeakyReLU
import numpy as np


def ergo_lstm(lstm_dim=64, dropout_rate=0.1, max_len=22, feat_dim=20):
    """
    ERGO-style model adapted to the same input format as nettcr_one_chain.

    Inputs:
      - TCR sequence: real-valued encoding of shape (max_len, feat_dim) per residue.
      - Epitope sequence: same shape.

    Architecture:
      1. Bidirectional LSTM encoder for both inputs (hidden size = lstm_dim).
      2. Concatenate final states -> vector of size 4*lstm_dim.
      3. MLP: Dense -> LeakyReLU -> Dropout -> Dense(sigmoid).
    """
    # Define inputs matching nettcr_one_chain
    tcr_input = Input(shape=(max_len, feat_dim), name='TCR_input')  # -> (batch, 22, 20)
    pep_input = Input(shape=(max_len, feat_dim), name='Epitope_input')  # -> (batch, 22, 20)

    # Bidirectional LSTM encoders over feature dimension
    tcr_enc = Bidirectional(
        LSTM(lstm_dim, return_sequences=False, dropout=dropout_rate),
        name='TCR_biLSTM'
    )(tcr_input)  # -> (batch, 2*lstm_dim)

    pep_enc = Bidirectional(
        LSTM(lstm_dim, return_sequences=False, dropout=dropout_rate),
        name='Epitope_biLSTM'
    )(pep_input)  # -> (batch, 2*lstm_dim)

    # Concatenate encoded representations
    merged = Concatenate(name='concat_encodings')([tcr_enc, pep_enc])
    # merged shape: (batch, 4*lstm_dim)

    # MLP classifier
    mlp_dim = 4 * lstm_dim
    hidden_units = int(np.sqrt(mlp_dim))
    hidden = Dense(hidden_units, name='hidden_dense')(merged)
    hidden = LeakyReLU(name='hidden_activation')(hidden)
    hidden = Dropout(dropout_rate, name='hidden_dropout')(hidden)

    # Output layer
    output = Dense(1, activation='sigmoid', name='binding_probability')(hidden)

    model = Model(inputs=[tcr_input, pep_input], outputs=output, name='ERGO_LSTM')
    return model
